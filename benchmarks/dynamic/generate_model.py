...
def main(FLAGS):
    ...
    if test_type == 'atomic_types':

        for i in range(atomic_types):
            generator.write_new_atomic_model(i)

        for i in range(atomic_types,atomics):
            generator.write_existent_atomic_model(atomic_types - 1, i) # repeat the last model
    ...
    generator.write_coupled_model(couples + 1, atomics, couples, ic)
    generator.close_model(couples + 1)
